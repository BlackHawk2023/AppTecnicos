...
<body>
  ...
  <script>
    ...
    // Validate material input in real-time without checking stock
    const validateMaterialStock = async (
        ...
        if (unidadMedida === 'SERIALIZADO') {
            const numericValue = parseInt(serieOCantidad);
            const isLikelyQuantity = !isNaN(numericValue) && serieOCantidad.length <= 4 && numericValue <= 1000;
            if (isLikelyQuantity) {
                updateMaterialItem(type, itemId, 'error', 'Ingresó una cantidad. Por favor seleccione un material por unidad.');
                return;
            }
